...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < n; ++i) data[i] = xorshift64(&state);
    qsort(data, n, sizeof(uint64_t), cmp_u64);
    uint64_t checksum = 0;
    for (size_t i = 0; i < n; i += 1024) checksum ^= data[i];
    double seconds = now_seconds() - start;
    print_result_csv("mixed_sort", "baseline_qsort", seconds, (double)(checksum % 1000000000ULL), "checksum_mod");
    free(data);
    return 0;
}
